...
up_amp <- Q_amp[2]+1.5*iqr_amp
low_amp <- Q_amp[1]-1.5*iqr_amp
...
up_unamp <- Q_unamp[2]+1.5*iqr_unamp
low_unamp <- Q_unamp[1]-1.5*iqr_unamp
...
write_tsv(C_bed_repeat_unamped, "C_bed_repeat_unamped_no_outlier.tsv")
write_tsv(C_bed_repeat_amped, "C_bed_repeat_amped_no_outlier.tsv")


#Map the uanmplified coverage with respect to repeat content.
map_unamp_c_ele_no_outliers <- ggplot(C_bed_repeat_unamped, aes(x = position)) + 
  geom_area(aes(y = X4/100), fill = "blue", outline.type = "lower", alpha = 0.6) + #Repeat_percentage, notice the transformation, this is so that it can be added a second axis.
  ...
  coord_cartesian(ylim = c(0,1.0)) + #This value will need to be changed IF you removed outliers, if you don,t the graph has a few 100kb blocks with coverage that is too high to properly view the graph.
  theme_cowplot() +
  scale_y_continuous(name = "Normalized Mean Coverage",
                     sec.axis = sec_axis(trans=~.*100,name = "Repeat Percentage [%]")) + #Adds a second y-axis on the right which is a transformation of the first one, the relationship should be inverse.
  scale_x_continuous(
...
map_unamp_c_ele_no_outliers

#Same as above just with amplified coverage.
map_amp_c_ele_no_outliers <- ggplot(C_bed_repeat_amped, aes(x = position)) + 
  geom_area(aes(y = X4/100), fill = "blue", outline.type = "lower", alpha = 0.6) +
  ...
  theme_cowplot() +
  scale_y_continuous(name = "Normalized Mean Coverage",
                     sec.axis = sec_axis(trans=~.*100,name = "Repeat Percentage [%]")) +
  scale_x_continuous(
...
map_amp_c_ele_no_outliers


#Will map repeats with respect to GC content. 
gc_content_c_ele_no_outliers <- ggplot(C_bed_repeat_amped, aes(x = position)) +
...
gc_content_c_ele_no_outliers

#Mapping amplified coverage with respect to GC content on a secondary axis.
map_amp_c_ele_gc_no_outliers <- ggplot(C_bed_repeat_amped, aes(x = position)) + 
  geom_area(aes(y = ((GC_content- 30)/20)), fill = "orange3", outline.type = "lower", alpha = 0.6) + #Mapping GC content, but we are doing an extra transformation on the axis so that GC content starts at the minimum of the dataset.
  ...
  coord_cartesian(ylim = c(0, 1.0)) + #Again this will need to be changed if outliers are added.
  theme_cowplot() +
  scale_y_continuous(name = "Normalized Mean Coverage",
                     sec.axis = sec_axis(trans = ~ .* 20 + 30, name = "GC Percentage [%]", #Notice the inverse transformation on the secondary axis.
  ...
  #   linetype = "dotted"
  # )
map_amp_c_ele_gc_no_outliers

#Mapping amlified coverage with respect to GC content on a secondary axis as above,
map_unamp_c_ele_gc_no_outliers <- ggplot(C_bed_repeat_unamped, aes(x = position)) + 
  geom_area(aes(y = ((GC_content- 30)/20)), fill = "orange3", outline.type = "lower", alpha = 0.6) +
  geom_area(aes(y = normal_mean), fill = "gray", outline.type = "lower", alpha = 0.6) +
  geom_vline(data = unique_unamp, aes(xintercept = position), linetype = "dotted", alpha = 0.8, color = "black") + 
  coord_cartesian(ylim = c(0, 1.0)) +  
  theme_cowplot() +
  scale_y_continuous(name = "Normalized Mean Coverage",
                     sec.axis = sec_axis(trans = ~ .* 20 + 30, name = "GC Percentage [%]",
...
map_unamp_c_ele_gc_no_outliers / map_amp_c_ele_gc_no_outliers

map_unamp_c_ele_no_outliers / map_amp_c_ele_no_outliers
...
gc_repeat_point_amp_no_outliers <- ggplot(C_bed_repeat_amped, aes(x = GC_content, y = normal_mean)) +
  ...
  coord_cartesian(xlim = c(25,45), ylim = c(0,1.0)) #Again this needs to be changed if you removed outliers.

gc_repeat_point_amp_no_outliers
...
#Make the point graph showing GC, conetn, coverage, and repeat percentage all the same graph.
gc_repeat_point_unamp_no_outliers <- ggplot(C_bed_repeat_unamped, aes(x = GC_content, y = normal_mean), size = 1) +
  ...
  coord_cartesian(xlim = c(25,45), ylim = c(0,1.0))
gc_repeat_point_unamp_no_outliers


#Save all the ggplot objects we created
save(map_unamp_c_ele_no_outliers, file = "ggplot_objects/map_unamp_c_ele_no_outliers")
save(map_amp_c_ele_no_outliers, file = "ggplot_objects/map_amp_c_ele_no_outliers")
save(gc_content_c_ele_no_outliers, file = "ggplot_objects/gc_content_c_ele_no_outliers")
save(gc_repeat_point_amp_no_outliers, file = "ggplot_objects/gc_repeat_point_amp_no_outliers")
save(gc_repeat_point_unamp_no_outliers, file = "ggplot_objects/gc_repeat_point_unamp_no_outliers")
save(map_unamp_c_ele_gc_no_outliers, file = "ggplot_objects/map_unamp_c_ele_gc_no_outliers")
save(map_amp_c_ele_gc_no_outliers, file = "ggplot_objects/map_amp_c_ele_gc_no_outliers")
