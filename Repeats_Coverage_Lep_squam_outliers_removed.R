...
Lep_squam <- Lep_squam %>% filter(X5 < up, X5 > low)
...
write_tsv(Lep_squam, "Lep_squam_no_outliers.tsv")

#Map the coverage with respect to repeat content.
map_lep_squam_no_outliers <- ggplot(Lep_squam_filtered, aes(x = position)) + 
  geom_area(aes(y = X4/100), fill = "olivedrab", outline.type = "lower", alpha = 0.6) + #Repeat_percentage, notice the transformation, this is so that it can be added a second axis.
  ...
  #   linetype = "dotted"
  # )
map_lep_squam_no_outliers

#Mapping coverage with respect to GC content on a secondary axis.
map_lep_squam_gc_no_outliers <- ggplot(Lep_squam_filtered, aes(x = position)) + 
  geom_area(aes(y = ((GC_content- 39)/15)), fill = "orange3", outline.type = "lower", alpha = 0.6) + #Mapping GC content, but we are doing an extra transformation on the axis so that GC content starts at the minimum of the dataset.
  ...
  theme_cowplot() +
  scale_y_continuous(name = "Normalized Mean Coverage",
                     sec.axis = sec_axis(trans = ~ .* 15 + 39, name = "GC Percentage [%]", #Notice the inverse transformation on the secondary axis.
...
map_lep_squam_gc_no_outliers 

#Will map repeats with respect to GC content. 
gc_content_lep_squam_no_outliers <- ggplot(Lep_squam_filtered, aes(x = position)) +
...
gc_content_lep_squam_no_outliers
...
gc_repeat_point_lep_no_outliers <- ggplot(Lep_squam, aes(x = GC_content, y = normal_mean)) +
  ...
  coord_cartesian(xlim = c(25,60), ylim = c(0,1.0))

gc_repeat_point_lep_no_outliers


#Save all the ggplot objects we created
save(gc_repeat_point_lep_no_outliers, file = "ggplot_objects/gc_repeat_point_lep_no_outliers")
save(map_lep_squam_no_outliers, file = "ggplot_objects/map_lep_squam_no_outliers")
save(gc_content_lep_squam_no_outliers, file = "ggplot_objects/gc_content_lep_squam_no_outliers")
save(map_lep_squam_gc_no_outliers, file = "ggplot_objects/map_lep_squam_gc_no_outliers")
